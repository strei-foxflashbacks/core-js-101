 ...
 */
const extractNameFromTemplate = (value) => value.substring(7).replace(/!/, '');
...
const getRectangleString = (width, height) => {
  ...
  let count = 0;
  let res = '';
  res += `${shape.topLeft}${(shape.horizontal).repeat([width - 2])}${shape.topRight}\n`;
  while (count < height - 2) {
    res += `${shape.vertical}${(shape.inner).repeat([width - 2])}${shape.vertical}\n`;
    count += 1;
  }
  res += `${shape.botLeft}${(shape.horizontal).repeat([width - 2])}${shape.botRight}\n`;
  return res;
};
...
const getCardId = (value) => {
  ...
  result += markingsValues[value.charAt(value.length - 1)];
  if (value[0] in nonNumeric) {
    result += nonNumeric[value[0]];
  } else {
    result += Number(value.replace(/(♣|♦|♥|♠)/, '')) - 1;
  }
  return result;
};
